...
const FLIGHT_ALTITUDE_TRIGGER_THRESHOLD = 4000; // in m
...
class TptyTrip {
  ...
  /**
   * Checks whether the current ping has a different country than the previous country
   * 
   * @param {Object} previousPing        - Previous ping details
   * @param {Object} currentPing         - Current ping details
   */
  ...
  /**
   * Returns whether the ping altitude is higher than the threshold
   * 
   * @param {Object} ping                - Ping data
   */
  highAltitude(ping) {
    return (ping.altitude && ping.altitude > FLIGHT_ALTITUDE_TRIGGER_THRESHOLD);
  }

  /**
   * Returns whether the ping distance is higher than the threshold
   * 
   * @param {Object} ping                - Ping data
   */
  longDistance(ping) {
    return (ping.distance && ping.distance > FLIGHT_DISTANCE_TRIGGER_THRESHOLD);
  }

  /**
   * Returns whether the ping is in a flight
   * 
   * @param {Object} ping                - Ping data
   */
  isInFlight(ping) {
    return (this.highAltitude(ping) || this.longDistance(ping) || ping.city === null);
  }

  hasVisited(currentPing) {
    return (currentPing.distance >= TIME_REQUIRED_VISIT && !currentPing.transport)
  }

  /**
   * Merges the timestamp, photos, venue and transport of two pings
   * 
   * @param {Object} current             - Ping that receives the properties
   * @param {Object} withPrevious        - Ping that gets copied
   * @param {Object?} options            - Additional options
   *      @param {Boolean} withCoords         - Whether to merge the latitude, longitude, country and city too
   */
  ...
  getAllTrips() {
    return store.User.user?.trips;
  }

  getLastTrip() {
    try {
      const trips = this.getAllTrips();
      return (trips) ? trips[trips.length-1] : null;
    } catch(err) {
      logger.error('TptyTrip.getLastTrip >', err.message);
    }
  }
  ...
  getLastPingOfTrip(trip) {
    try {
      ...
      return (pings) ? pings[pings.length-1] : null;
    } catch(err) {
      logger.error('TptyTrip.getLastPingOfTrip >', err.message);
    }
  }
  ...
  async OnLocationPing(ping, simTimestamp) {
    try {
      ...
      if (trip.finishedAt) {
        throw new Error('Blocked ping after trip was finished!');
      }
      ...
      logger.error('TptyTrip.OnLocationPing >', err.message);
    }
  }
    ...
    try {
      ...
      if (trip && !trip.finishedAt) {
        ...
        await Realm.write(() => {
          ...
          trip.finishedAt = lastPing.timestamp;
        });
      }
      if (!simTimestamp) {
      ...
      logger.error('TptyTrip.OnRegionEnter >', err.message);
    }
  }
  ...
  async OnRegionLeave(region, simTimestamp) {
    try {
      ...
      logger.error('TptyTrip.OnRegionLeave >', err.message);
    }
  }

  async parseTrips(updateFn) {
    ...
    for (let t = 0; t < trips.length; t++) {
      await this.parsePings(trips[t], updateFn);
    }
  }

  async parsePings(trip, updateFn) {
    ...
    try {
      ...
      for (let p = 0; p < pings.length; p++) {
        ...
        if (!currentPing.parsed) {
          try {
            ...
            currentPing.transport = (isLastPing || this.isInFlight(currentPing));
            ...
            if (this.hasVisited(currentPing)) {
              ...
              if (venueResponse) {
                if (previousPing.venue && previousPing.venue.venueId === venueResponse.venueId) {
                  ...
                  this.mergePings(currentPing, previousPing, { withCoords: true });
                  pings.splice(--p, 1);
                } else {
                  // Use the venue data
      ...
      logger.error('TptyTrip.parsePings >', err.message);
  ...
  async _sim_parseMedia(assets=[]) {
    try {
      ...
      for (let i = 0; i < assets.length; i++) {
        ...
        // Skip the photo if it has been taken at a high altitude
        if (this.highAltitude(currentPing)) {
        ...
        if (currentTrip && (this.isHome(homeCoords, meta.coords) || (previousPing && previousPing.country !== currentPing.country))) {
          ...
          if (minimumHours) {
            ...
            currentTrip.finishedAt = lastPing.timestamp;
          } else {
            trips.pop();
          }
          currentTrip = null;
      ...
      logger.error('TptyTrip._sim_parseMedia >', err.message);
    }
  }
  ...
  async parseMedia() {
    try {
      ...
      logger.error('TptyTrip.parseMedia >', err.message);
    }
  }
}
